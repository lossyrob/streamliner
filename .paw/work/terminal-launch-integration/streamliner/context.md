...
- `launch-prompt-profiles` / issue #33 / PR #41 plus follow-up commits on `main`: the graph launch dialog and backend preparation route collect launch instructions, prompt-profile snippets, CLI args, and terminal preferences; run a fully capable SDK PAW init session; install `.paw/work/<work-id>/streamliner/context.md`; expose bounded `WorkflowContext.md` review/editing; and return a structured terminal handoff without starting the visible worker. The init session now filters builder text and returns `kickoffAdditionalInstructions` only for worker-startup guidance that was not already encoded into `WorkflowContext.md`.
...
- `src/server/launch-preparation.ts` defines `PawLaunchHandoff` and `preparePawLaunch`. The returned handoff includes `cwd`, `branch`, `pawWorkDir`, `workflowContextPath`, `streamlinerContextPath`, `kickoffPrompt`, optional `kickoffAdditionalInstructions`, `cliArgs`, `environment`, `sessionStateRoot`, `launchMetadata`, and `contextPackage`.
...
- `src/server/node-launch-record-store.ts`, `src/server/routes/node-launch-records.ts`, and `src/components/NodeInspector.tsx` persist and display the latest PAW launch preparation metadata for a graph node, including worktree/context paths and live present/missing status. This record is preparation metadata, not terminal-launch completion, but it is a useful seam for preventing duplicate launches once #44 adds real launch/claim lifecycle state.
...
The current kickoff prompt from preparation follows the Streamliner PAW-lite launch template: it identifies the Streamliner node session, includes the GitHub issue when one exists, lists `WorkflowContext.md` and `streamliner/context.md`, instructs the worker to load `paw-lite`, and appends only filtered `kickoffAdditionalInstructions` that were not already encoded into `WorkflowContext.md`. It also carries launch metadata including nonce/claim fields, but issue #44 explicitly requires the final launched prompt to support the Tier 1 nonce scanner. After creating the launch claim, wrap or augment the prepared prompt so the actual Copilot `user.message` contains the canonical line from `kickoffNonceLine(nonce)` while preserving the prepared instructions and context paths.

...
After previewing PR #41, the dialog briefly expanded into a front end for the PAW `WorkflowContext.md` header fields. That richer form was then deferred to follow-up issue #43 because it should use PAW-owned metadata for presets, specialists, constraints, and derived fields. The implemented PR scope returns to a natural-language PAW workflow instructions textarea. Streamliner stages the worker-facing context in local state before PAW init, then runs a fully capable SDK `paw-init` session with Copilot CLI-style repository, shell, GitHub, configured MCP, and custom-tool access. PAW init installs the staged context into the PAW work directory through Streamliner's `complete_paw_init` completion tool. If PAW init asks a clarification question, launch preparation reports a typed error instead of waiting indefinitely.
...
- **Context installation tool**: Provide `complete_paw_init` as the structured SDK completion tool. The PAW init session can use built-in tools, and this tool writes the PAW-derived WorkflowContext, copies the staged context into the PAW work dir, and ensures `Additional Inputs` records the installed Streamliner context.

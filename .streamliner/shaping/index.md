...
| [Streamliner Plugin Role Skills](candidates/streamliner-agent-skill-context.md) | Shaped | Add a small `streamliner` entry skill plus designer, formation, orchestrator, and worker role skills; export stable role/context rules for Telex binding and artifact-root discovery. |
...
| [Git-backed Artifact Ledger & Sync](candidates/git-backed-artifact-sync.md) | Shaped | Support a same-repository `streamliner-artifacts` branch/worktree, one artifact-root resolver, deterministic synchronization, and Telex-routed conflict attention across machines and builders. |
| [Telex-backed Session Actor Control Plane](candidates/session-actor-control-plane.md) | Shaped | Map Streamliner project/workstream/node roles onto released Telex addresses and stations, with bound orchestrators, lifecycle routing, field reports, and durable-artifact promotion. |

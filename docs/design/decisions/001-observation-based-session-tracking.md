...
Prior art: Donna's `SessionWatcher` originally required agents to call MCP tools (`donna_register`, `donna_done`) to self-report status. Agents frequently skipped these calls, leaving Donna blind to session activity. Donna moved to observing Copilot CLI's own session state files directly, which proved far more reliable.
...
**MCP tool registration** — sessions call `register`/`done` tools on a Streamliner MCP server. Rejected: Donna tried this approach and found agents frequently skipped the calls. The MCP handshake also adds 10-15 seconds of latency per lifecycle event. Signal files (<100ms) are better when explicit signals are needed.
...
- Streamliner depends on Copilot CLI's undocumented session state file format. Changes to that format require updating the watcher. This is an accepted trade-off: the format is stable enough for production use (Donna has run on it for months), and the alternative (requiring agent cooperation) is less reliable.
...
- The observation model naturally extends to remote sessions by polling remote session state directories over SSH, matching Donna's existing devbox support pattern.

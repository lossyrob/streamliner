...
code_paths:
  ...
  - src/workstream-positions-contract.ts
  - src/server/workstream-positions.ts
...
| 14 | `externalDependsOn[].id` values are unique within each node |
| 15 | External targets cannot point at the owning node |

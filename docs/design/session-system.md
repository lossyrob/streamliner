...
The standalone `/profiles` page is the primary management surface for these local PAW prompt profiles. It shares the same application-level profile state as the launch dialog, refreshes from the local API without browser caching, and supports create, edit, duplicate, copy-instructions, and delete actions. Mutations update shared in-memory state immediately, so a profile changed in `/profiles` is visible the next time the launch dialog opens in the same browser session.

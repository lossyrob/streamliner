...
| Command composition | Microsoft documents semicolon-separated `wt` commands and PowerShell-specific escaping patterns for passing semicolons to `wt`. | Launch/relaunch implementation must avoid naive shell-string composition and must handle both escaping layers: shell-to-process invocation and Windows Terminal's own command-stream parsing of semicolons and quoted values. |
...
| `tabColor` | Registry row `color` | Accept only `#RGB` or `#RRGGBB` after palette-token resolution. Unknown palette tokens, named colors, alpha hex, malformed hex, and `null` are treated as absent color. Palette resolution belongs to the shared registry presentation layer; relaunch must reuse that resolver rather than define a terminal-only palette. |
...
The exact process-spawn API is an implementation detail, but the command must be constructed safely across two parsing layers. First, the caller must avoid shell interpolation by using argument-vector process creation or equivalent quoting for the shell boundary. Second, values that enter Windows Terminal's command stream must be escaped or quoted for `wt.exe` itself: `;` separates Windows Terminal commands even after the shell boundary, and embedded quotes, leading/trailing whitespace, and path characters in registry-derived `title`, `cwd`, kickoff prompt, or command text must not be allowed to split or rewrite the intended `new-tab` command.
